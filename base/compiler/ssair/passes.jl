...
function lift_comparison_leaves!(@nospecialize(tfunc),
    ...
    typeconstraint = widenconst(compact_exprtype(compact, val))
    ...
    cmp = compact_exprtype(compact, target)
    lifted_leaves = nothing
    for leaf in leaves
        result = tfunc(compact_exprtype(compact, leaf), cmp)
        if isa(result, Const)
